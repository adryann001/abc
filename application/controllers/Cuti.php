...
class Cuti extends CI_Controller {
    ...
    public function setujui_cuti_super_admin()
    {
        $id_cuti = $this->input->post("id_cuti");
        $id_user = $this->input->post("id_user");
        $alasan_verifikasi = $this->input->post("alasan_verifikasi");

        // Logika untuk menentukan admin yang akan menangani permohonan
        $id_admin = $this->pilih_admin_terkait();

        // Simpan data perubahan status permohonan ke database
        $hasil = $this->m_cuti->confirm_cuti($id_cuti, $id_status_cuti, $alasan_verifikasi, $id_admin);
	...
	private function pilih_admin_terkait()
	{
		// Misalnya, Anda mengambil admin berikutnya berdasarkan urutan ID
		$admin_terkait = $this->m_user->get_next_admin();
	
		if($admin_terkait) {
			return $admin_terkait['id_admin']; // Mengembalikan ID admin terkait
		} else {
			// Jika tidak ada admin yang tersedia, Anda bisa menangani ini sesuai kebutuhan aplikasi Anda
			// Misalnya, mengembalikan ID admin default atau menampilkan pesan kesalahan
			return null;
		}
	}
}